...
static void vera_put_char(unsigned char ch)
{
	...
	if (ch == 0x1C) { if (ctl->cursor_x > 0) --ctl->cursor_x; vera_touch_cursor(); return; }
	if (ch == 0x1D) { if (ctl->cursor_x < VERA_SCREEN_COLS - 1) ++ctl->cursor_x; vera_touch_cursor(); return; }
	if (ch == 0x1E) { if (ctl->cursor_y > 0) --ctl->cursor_y; vera_touch_cursor(); return; }
	if (ch == 0x1F) { if (ctl->cursor_y < VERA_SCREEN_ROWS - 1) ++ctl->cursor_y; vera_touch_cursor(); return; }
